...
| `preferred_companion` | enum | alone / spouse / family_with_kids / friends / parents / colleagues / nobody | 그 외 값은 분석 단계에서 fail로 분류 |
| `recommend_to` | enum | spouse / family / friends / colleagues / general_public / no_one | 그 외 값은 분석 단계에서 fail로 분류 |
...
- 동행자 선호 분포 (preferred_companion) × marital_status / family_type 교차 — 페르소나 sensitivity 검증
- 추천 대상 분포 (recommend_to) — RLHF 정중성 강한 모델은 "general_public"으로 mode collapse 가설
