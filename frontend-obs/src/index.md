...
## 본 라운드 — N=${fmt.format(meta.n_total)}

<div class="lede">
한국 인구통계 분포 합성 페르소나 <strong>${fmt.format(meta.n_total)}명</strong>에 던진 결과입니다. JSON 응답 성공 <strong>${fmt.format(meta.n_ok)}건</strong> 중 schema 정의 텍스트가 응답값으로 그대로 회신된 ${fmt.format(meta.n_schema_echo)}건을 제외한 <span class="valid-n-callout">valid ${fmt.format(meta.valid_n)}건</span>이 본 분포·교차분석의 통계 베이스입니다.<br>
시나리오 주제·맥락·찬반 논거·질문지·응답 schema 전체는 <strong>아래 「시나리오 기획안」</strong>을 펼쳐 확인하세요.
</div>
...
## ③ 분석 표 — analysis tables

<div class="muted">보고서 합성 LLM이 본 측정의 응답 schema(질문 type)와 raw 응답을 직접 보고 어떤 표가 본 라운드를 가장 잘 보여주는지 자율 결정한 표입니다. 표 종류·축·집계 방식 모두 LLM 산출물이며 측정마다 달라집니다.</div>

```js
const tables = round.insight.analysis_tables || [];

function mdTableToHtml(md) {
  const lines = (md || "").trim().split("\n").filter((l) => l.trim());
  if (lines.length < 2) return html`<pre>${md}</pre>`;
  const PH = "__KK_PIPE_ESC__";
  const parseRow = (line) =>
    line.replace(/^\||\|$/g, "")
        .replace(/\\\|/g, PH)
        .split("|")
        .map((s) => s.trim().split(PH).join("|"));
  const header = parseRow(lines[0]);
  const align = parseRow(lines[1]).map((s) => {
  ...
  return html`<table>
    <thead><tr>${header.map((h, i) => html`<th style="text-align: ${align[i]}">${h}</th>`)}</tr></thead>
    <tbody>${rows.map((r) => html`<tr>${r.map((c, i) => html`<td style="text-align: ${align[i]}">${c}</td>`)}</tr>`)}</tbody>
  </table>`;
}
```

<div>
${tables.length === 0 ? html`<div class="muted">analysis_tables 비어 있음</div>` : tables.map((t, i) => html`
  <div class="analysis-table-card">
    <h3 style="margin-top: 0">${t.title || `표 ${i + 1}`}</h3>
    ${mdTableToHtml(t.markdown || "")}
  </div>
`)}
</div>
...
<div class="muted">
knowing-koreans · <strong>v0.3.2</strong> · 2026-05-06 · run_id <code>${meta.run_id || "-"}</code><br>
