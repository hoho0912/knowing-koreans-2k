...
# 4섹션 출력 스키마 — 모드 A 단일 호출과 모드 B 단계 5(합성)에서 공통 사용
...
- key_findings: 5~8개. 첫 열은 중립적 차원 이름만, 분석 결론·수치는 content.
...
본 응답은 보고서의 "해석·인사이트" 부분만 담당합니다. 측정 개요, 페르소나
분포, 응답자 속성 축별 분포 표, 원본 질문·스키마·명세는 별도 코드에서
결정론적으로 생성되어 본 응답의 앞뒤에 자동으로 붙습니다. 따라서 본
응답에서는 통계 표를 다시 그릴 필요 없고, 4섹션 JSON으로만 답하세요.

""" + _INSIGHT_COMMON_PRINCIPLES


INSIGHT_SINGLE_USER_TEMPLATE = """이번 측정의 결정론적 팩트 자료입니다.
이 팩트를 그대로 다시 표로 그리지 마세요 — 코드가 본 응답 위아래에
측정 개요·축별 분포·부록을 자동으로 붙입니다. 본 응답에는
**해석·가설·인사이트**만 4섹션 JSON으로 담아 주세요.
