...
─────────────────       ──────────────         ────────────         ────────────         ────────
Nemotron-Personas-      scenario/context.md    OpenAI / Ollama /    backend/results/      Observable
Korea (parquet 9샤드)   + scenario/question    OpenRouter            *.csv (누적)         Framework
       ↓                       ↓                     ↓                    ↓                  ↓
   N명 샘플링          페르소나 + 시나리오        JSON 응답            CSV 1행 +          정적 site
  (시드 고정)          → 단일 프롬프트         (vote/reason 등)      raw JSON 1파일       (frontend-obs/)
```
...
| 규칙 | context.md는 수치·출처 제거된 중립 톤 (LLM 응답 앵커링 회피) |
...
### [5] 시각화 — `frontend-obs/` (Observable Framework)

| 항목 | 값 |
|------|---|
| 입력 | `frontend/data/<scenario_id>_*/` (backend 산출물 그대로) — Python loader(`src/round1.json.py`)가 빌드 시 JSON으로 변환 |
| 출력 | 정적 사이트 (`frontend-obs/dist/`) — 100% 누적 막대(Q1·Q2·Q3) + lollipop(Q4·Q5) + 페르소나 속성 × Q1 crosstab 9종 + 인사이트 합성 결과 |
| 박물관 추가 | 시나리오 셀렉터, 다양성 지표(같은 demographic 내 응답 분산), 시계열 비교(M4부터) |
| 호스팅 | GitHub Pages (`.github/workflows/deploy.yaml`) — main 브랜치 push 시 Observable Framework 빌드 + 배포 자동 |
| 검증 | `npm --prefix frontend-obs run dev` (localhost:3000) 또는 `npm --prefix frontend-obs run build` 후 dist/ 검증 |
