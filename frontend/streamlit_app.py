"""
knowing-koreans — exhibition_appeal 시각화 prototype (stlite)

페르소나 5명 × 5모델 = 25건 응답 데이터를 기반으로 모델별 편향과
페르소나 차원 cross-tab을 보여준다. vuski/persona-million의 정치 분야
방법론을 박물관·문화 분야로 옮긴 첫 단면.
"""
...
st.set_page_config(
    page_title="knowing-koreans — 전시 호감도",
    layout="wide",
...
# ---------- 헤더 ----------
st.title("knowing-koreans — 전시 콘셉트 호감도")
st.caption(
...
with st.expander("📋 전시 기획안", expanded=False):
    st.markdown(
        """
**제목** — 묻힌 그릇들은 아직 끝나지 않았다
**부제** — Where Things Linger: 쓰임·폐기 그리고 다시 불러진 것들
**기간** — 2026.05.06 ~ 2026.07.18
**장소** — 한양대학교박물관 (서울 성동구)
**관람료** — 무료 관람 (한양대학교박물관 상시 무료, 2026 박물관·미술관 주간 일환)

깨지고, 버려지고, 잊혔다고 여겨진 것들은 어디에도 닿지 못한 채 오래 머물러 있습니다.
한때 식탁 위에 놓였던 시간, 누군가의 손을 지나며 남겨진 온기, 그리고 어느 순간 내려진
'버림'의 판단까지 — 그릇은 사라지지 않고 다른 자리에서 계속됩니다.

이 전시는 서로 다른 이유로 놓여진 네 개의 장면을 따라 묻고 또 묻습니다.
무엇이 남겨지고, 무엇이 버려지는가. 그리고 우리는 무엇을 끝났다고 말할 수 있는가.

**주요 볼거리**
- '쓰임 → 폐기 → 다시 불러짐'의 흐름을 따라 구성된 네 개의 장면
- 일상의 그릇이 거쳐가는 시간 — 식탁 위의 시간, 손에서 손으로 전해진 온기, '버림'의 판단까지
- 이태수 작가 아티스트 토크 (5월 6일 14:00)
- 오프닝 행사 (5월 6일 16:00)
- 한국박물관협회·ICOM Korea 협력, 한양대학교박물관 87주년 기념 전시
"""
    )
...
# ---------- 모델별 호감도 (핵심: Models aren't polls) ----------
st.subheader("① 모델별 호감도 — *Models aren't polls* 시각화")
st.caption(
    "vuski/persona-million의 핵심 비판: 같은 페르소나·같은 질문에도 모델마다 "
